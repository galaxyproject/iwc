...
- Add a subworkflow that allows to run eggNOG with different types of sequences (protein, CDS, genome and metagenome). Needs to be in a subworkflow since the option to change eggNOG mode is bound to a section segment in the wrapper which can not be changed by linking a input to it 
